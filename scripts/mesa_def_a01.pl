...
# contains definitions specific to the A01 HL7 message.
...
sub getQueries {
    ...
    my $query = "SELECT patid, nam, issuer, datbir, sex, addr, pataccnum, " .
        "patrac, visnum, patcla, asspatloc, attdoc, admdoc, admdat, admtim, " .
        "refdoc FROM patient_visit WHERE viskey = '".$param->{"viskey"}."'";
    return ($query);
}
