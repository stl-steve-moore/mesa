/* cgi-lib.c - C routines that make writing CGI scripts in C a breeze
   ...
   $Id: cgi-lib.c,v 1.1 2001/05/25 15:27:12 smm Exp $
...
int parse_form_encoded(llist* entries)
{
  ...
  getlineLocal(buffer,BUFSIZ);
  /* now start parsing */
  while ((bytesread=getlineLocal(buffer,BUFSIZ)) != 0) {
    start = 1;
    ...
    *tempstr = '\0';
    ...
    while (getlineLocal(buffer, BUFSIZ) > 1) {
    ...
    j = 0;
    while (!done) {
      bytesread = getlineLocal(buffer,BUFSIZ);
      buffer[bytesread] = '\0';
