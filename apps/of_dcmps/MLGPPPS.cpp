...
MLGPPPS::MLGPPPS(MDBOrderFillerBase& OrderFiller, const MString& storageDir) :
  mOrderFiller(OrderFiller),
  mStorageDir(storageDir + "/gppps")
{
}
			       ...
			       MString& directoryName)
{
  ...
  logClient.log(MLogClient::MLOG_VERBOSE,
	       ...
	       "MLGPPPS::handleNCreateDataSetCommand",
	       __LINE__,
	       MString("MPPS status stored in ") + gpppsPath);
			  ...
			  MString& directoryName)
{
  ...
  logClient.log(MLogClient::MLOG_VERBOSE,
		...
		"MLGPPPS::handleNSetDataSet",
		__LINE__,
		MString("MPPS status updated ") + gpppsPath);
