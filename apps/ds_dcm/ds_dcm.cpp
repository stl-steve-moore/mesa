...
**			classes.
** Usage:		ds_dcm [-a] [-d FAC] [-i] [-m maxPDU] [-p] [-t trips] [-v] [-w] [-z sec] port
** Last Update:		$Author: smm $, $Date: 2009/03/06 18:41:50 $
** Source File:		$RCSfile: ds_dcm.cpp,v $
** Revision:		$Revision: 1.28 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1.28 $ $RCSfile: ds_dcm.cpp,v $";
...
runThread(void *arg)
{
  ...
  int status = (s->imageManager)->initialize(s->databaseName);
  if (status != 0) {
...
static void
writePIDFile(MConfigFile& configFile)
{
  MString s = configFile.value("SERVER_NAME");
  if (s == "") {
    cerr << "No value registered for SERVER_NAME in config file." << endl;
    ::exit(1);
  }
...
buildSOPHandlerVector(MConfigFile& configFile,
		      MDICOMReactor& reactor, MDBImageManager* imageManager,
		      MSOPHandlerVector& v)
{
  MFileOperations f;
  MString logDir = configFile.completePath("LOG_DIRECTORY");
  MString storageDir = configFile.completePath("STORAGE_DIRECTORY");
  MString mppsDir = configFile.completePath("MPPS_DIRECTORY");
  MString gpppsDir = configFile.completePath("GPPPS_DIRECTORY");
  MString scDir = configFile.completePath("STORAGECOMMIT_DIRECTORY");
  MString storageCommitRole = configFile.completePath("STORAGECOMMIT_ROLE");

  int storeCFindQueries = configFile.intValue("STORE_CFIND_QUERIES");
  MString queryDir = configFile.completePath("QUERY_DIRECTORY");
  if (storeCFindQueries)
    f.createDirectory(queryDir);
  MString imgMgrAETitle = configFile.value("AETITLE");

  MSOPHandler* echoHandler = new MSOPHandler;
  v.push_back(echoHandler);

  MLQuery* queryHandler = new MLQuery(imageManager, storeCFindQueries,
			queryDir, imgMgrAETitle);
  registerSOPClasses(*queryHandler, reactor, configFile, "CFIND_CLASSES");
  v.push_back(queryHandler);

  MLMove* moveHandler = new MLMove(imageManager, 1, queryDir);
  registerSOPClasses(*moveHandler, reactor, configFile, "CMOVE_CLASSES");
  v.push_back(moveHandler);
  ...
  if (mppsDir != "") {
    MLMPPS* ppsHandler = new MLMPPS(imageManager, logDir, mppsDir);
    registerSOPClasses(*ppsHandler, reactor, configFile, "MPPS_CLASSES");
    v.push_back(ppsHandler);
  }

  if (gpppsDir != "") {
    MLGPPPS* gpppsHandler = new MLGPPPS(imageManager, logDir, gpppsDir);
    registerSOPClasses(*gpppsHandler, reactor, configFile, "GPPPS_CLASSES");
    v.push_back(gpppsHandler);
  }

  if (storageCommitRole == "SCU") {
    MLStorageCommitmentSCU* storageCommitmentHandlerSCU =
      new MLStorageCommitmentSCU(*imageManager, scDir);
    registerSOPClasses(*storageCommitmentHandlerSCU, reactor, configFile, "STORAGE_COMMIT_CLASSES");
    v.push_back(storageCommitmentHandlerSCU);
  } else if (scDir != "") {
    MLStorageCommitment* storageCommitmentHandler =
      new MLStorageCommitment(imageManager, logDir, scDir);
    registerSOPClasses(*storageCommitmentHandler, reactor, configFile, "STORAGE_COMMIT_CLASSES");
    v.push_back(storageCommitmentHandler);
  }
...
int main(int argc, char **argv)
{
  ...
  if (logLevel != MLogClient::MLOG_NONE) {
    ...
    logClient.log(MLogClient::MLOG_VERBOSE,
		  "<no peer>", "ds_dcm<main>", __LINE__,
		  "Begin server process");
    cerr << "ds_dcm logging messages at level "
	 << logLevel
	 << " to "
	 << logFile
	 << endl;
  ...
  MDBImageManager* imageManager;
  if (dataModel == "PATIENT" && language == "Japanese") imageManager = new MDBImageManagerJapanese();
  else if (dataModel == "PATIENT") imageManager = new MDBImageManager();
  else if (dataModel == "STUDY") imageManager = new MDBImageManagerStudy();
  else {
    cout << "DATA_MODEL value should be one of PATIENT or STUDY" << endl
	 << "Your value is " << dataModel << endl;
    exit (1);
  }
...
//			scRole = DUL_SC_ROLE_DEFAULT;
	  ...
	  MSOPHandler* tmpHandler = reactor.recallHandler(requestedCtx->abstractSyntax);
	  if (tmpHandler->xferIVRLEOnly()){
	    logClient.logTimeStamp(MLogClient::MLOG_VERBOSE,
		"Trying to accept this SOP class, but only with IVRLE");
	    // e.g., Query handlers, MPPS, Storage Commitment.
	    cond = ::SRV_AcceptServiceClassWithOneXferSyntax(requestedCtx,
		scRole, service,
		DICOM_TRANSFERLITTLEENDIAN);
	  } else {
	    logClient.logTimeStamp(MLogClient::MLOG_VERBOSE,
		"Trying to accept this SOP class, server will try multiple xfer syntaxes");
	    cond = ::SRV_AcceptServiceClass(requestedCtx,
		scRole, service);
	  }
	  if (cond == SRV_NORMAL) {
	    classCount++;
...
{
    char usage[] = "\
Usage: ds_dcm [-c <title>] [-d FAC] [-f] [-i] [-j] [-l level] [-m maxPDU] [-p] [-r port] [-t trips] [-v] [-y] <config file>\n\
\n\
