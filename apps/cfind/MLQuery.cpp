...
MLQuery::handleCFindResponse(DUL_PRESENTATIONCONTEXT* ctx,
				 ...
				 int index)
{
  U16 s = (*message)->status;

  if (s == MSG_K_SUCCESS || s == MSG_K_C_FIND_COMPLETE ||
	s == MSG_K_C_FIND_MATCHCONTINUING ||
	s == MSG_K_C_FIND_MATCHCONTINUINGWARN ) {

  } else {
    cout << "Peer returned an error status in C-Find response: "
	 << setw(4) << setfill ('0') << hex << s << endl;
    return 0;
  }
  ...
  if ((*message)->dataSetType != DCM_CMDDATANULL) {
    ...
    cout << endl << "*----Query result " << mMsgnum++ << ":-------*" << endl;
