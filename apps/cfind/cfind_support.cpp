...
static char rcsid[] = "$Revision: 1.2 $ $RCSfile: cfind_support.cpp,v $";
...
static STRING_MAP m1[] = {
  ...
  { "MWL", DICOM_SOPMODALITYWORKLIST_FIND},
  { "GPWL", DICOM_SOPGPWORKLIST_FIND},
  { "UPS", "1.2.840.10008.5.1.4.34.4.3"}
};
...
translateQueryClass(const char* str)
{
  ...
  for (index = 0; index < (int)DIM_OF(m1); index++) {
    if (strcmp(str, m1[index].synonym) == 0)
      return m1[index].sopClass;
  }
