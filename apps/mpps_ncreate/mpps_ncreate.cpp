...
static char rcsid[] = "$Revision: 1.6 $ $RCSfile: mpps_ncreate.cpp,v $";
...
{
    char msg[] = "\
mps_ncreate [-a title] [-c title] [-p] [-v] node port <create file> <instance UID>\n\
\n\
...
int main(int argc, char **argv)
{
	...
	* association = NULL;
    ...
    cond = ::SRV_RequestServiceClass(DICOM_SOPCLASSMPPS, role,
				   &params);
    if (cond != SRV_NORMAL) {
	::COND_DumpConditions();
	::THR_Shutdown();
    ...
    reactor.registerHandler(&qHandler, DICOM_SOPCLASSMPPS);
    qHandler.sendNCreateRequest(&association, &params,
				DICOM_SOPCLASSMPPS,
				sopInstanceUID, &queryObj);
