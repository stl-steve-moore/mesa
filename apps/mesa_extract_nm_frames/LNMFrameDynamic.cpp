...
static char rcsid[] = "$Id: LNMFrameDynamic.cpp,v 1.4 2004/09/21 15:30:03 ssurul01 Exp $";

LNMFrameDynamic::LNMFrameDynamic ()
{
}

LNMFrameDynamic::~LNMFrameDynamic (void)
{
}

LNMFrameDynamic::LNMFrameDynamic (const LNMFrameDynamic& cpy)
{
}

void
LNMFrameDynamic::printOn(ostream& s) const
{
  s << "LNMFrameDynamic";
}

void
LNMFrameDynamic::streamIn(istream& s)
{
}
...
int
LNMFrameDynamic::extract(ofstream& f, MDICOMWrapper& w, MStringMap& m, ofstream& o)
{
  U32 frameSize = 0;
  U16 frameCount = 0;
  ...
  int phaseVectorLength;
  unsigned short* phaseVector = 0;
  if (w.getVector(phaseVectorLength, phaseVector, 0x00540030) != 0) {
    return 1;
  }
  int timeSliceVectorLength;
  unsigned short* timeSliceVector = 0;
  if (w.getVector(timeSliceVectorLength, timeSliceVector, 0x00540100) != 0) {
    return 1;
  }
  ...
  unsigned short fixedPhase = 0;
  unsigned short fixedTimeSlice = 0;
  MString x;
  x = m["ENERGY"];
  ...
  x = m["PHASE"];
  if (x != "") fixedPhase = x.intData();
  x = m["TIMESLICE"];
  if (x != "") fixedTimeSlice = x.intData();
  ...
  for (i = 0; i < energyWindowVectorLength; i++) {
    ...
    cout << energyWindowVector[i] << ' ' << detectorVector[i] << ' ' 
	 << phaseVector[i] << ' ' << timeSliceVector[i]<< endl;
    ...
    if (fixedPhase != 0 && fixedPhase!= phaseVector[i]) {
      matchingFrame = false;
    }
    if (fixedTimeSlice != 0 && fixedTimeSlice!= timeSliceVector[i]) {
      matchingFrame = false;
    }
    cout << fixedEnergyWindow << ' ' << fixedDetector << ' ' << fixedPhase << ' ' << fixedTimeSlice << 'x' << matchingFrame << endl;
