...
static char rcsid[] = "$Id: LNMFrameGatedTomo.cpp,v 1.1 2004/09/27 20:54:35 ssurul01 Exp $";

LNMFrameGatedTomo::LNMFrameGatedTomo ()
{
}

LNMFrameGatedTomo::~LNMFrameGatedTomo (void)
{
}

LNMFrameGatedTomo::LNMFrameGatedTomo (const LNMFrameGatedTomo& cpy)
{
}

void
LNMFrameGatedTomo::printOn(ostream& s) const
{
  s << "LNMFrameGatedTomo";
}

void
LNMFrameGatedTomo::streamIn(istream& s)
{
}
...
int
LNMFrameGatedTomo::extract(ofstream& f, MDICOMWrapper& w, MStringMap& m, ofstream& o)
{
  U32 frameSize = 0;
  U16 frameCount = 0;
  ...
  for (i = 0; i < energyWindowVectorLength; i++) {
    ...
    cout << energyWindowVector[i] << ' ' << detectorVector[i] << ' ' 
	 << rotationVector[i] << ' ' << angularViewVector[i] << ' '
    ...
    cout << fixedEnergyWindow << ' ' << fixedDetector << ' ' << fixedRotation << ' ' << fixedAngularView << ' ' << fixedRR << ' ' << fixedTimeSlot << 'x' << matchingFrame << endl;
