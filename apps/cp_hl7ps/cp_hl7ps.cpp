...
{
  char msg[] = "\
Usage: [-b base] [-d def] [-l level] [-s dir] port\n\
  This version is analysis mode only (capture and store HL7 messages in files\n\
...
Usage: [-a] [-b base] [-d def] [-l level] [-s dir] [-z db] port\n\
  ...
  -l   Set log level (0-4, default is 0: none) \n\
  -s   Set the local directory; default is $MESA_TARGET/logs \n\
  -z   Set databasename; default is chrgproc \n\
\n\
  port TCP/IP port used to accept connections";
...
{ 
  ...
  MString logDir(path);
  f.expandPath(path, "MESA_STORAGE", "chrgproc/hl7");
  ...
  if (logLevel != MLogClient::MLOG_NONE) {
    ...
    logClient.initialize(logLevel, logDir + "/cp_hl7ps.log");

    logClient.log(MLogClient::MLOG_ERROR,
                  "<no peer>", "cp_hl7ps<main>", __LINE__,
                  "Begin server process");
    cerr << "cp_hl7ps logging messages at level "
         << logLevel
         << " to "
         << logDir + "/cp_hl7ps.log"
         << endl;
  ...
  int shutdownFlag = 0;
  messenger = new MLMessenger(factory, // imageManagerDB, 
  ...
  if (acceptor.open(ACE_INET_Addr(port), &reactor) == -1)
    ACE_ERROR_RETURN((LM_ERROR,
		     ...
		     -1);
  ACE_DEBUG((LM_DEBUG, 
	     "(%P|%t) Firing up the ChargeProcessor server, port %d\n", port));
  ...
  ACE_DEBUG((LM_DEBUG,
	     "(%P|%t) Shutting down the ChargeProcessor server\n"));
  a.closeServerPID();
  delete messenger;
