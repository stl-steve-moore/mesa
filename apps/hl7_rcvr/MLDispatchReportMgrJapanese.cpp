...
MLDispatchReportMgrJapanese::MLDispatchReportMgrJapanese(const MLDispatchReportMgrJapanese& cpy) :
...
MLDispatchReportMgrJapanese::~MLDispatchReportMgrJapanese ()
{
}

void
MLDispatchReportMgrJapanese::printOn(ostream& s) const
{
  s << "MLDispatchReportMgrJapanese" << endl;
}

void
MLDispatchReportMgrJapanese::streamIn(istream& s)
{
  //s >> this->member;
}
...
MLDispatchReportMgrJapanese::MLDispatchReportMgrJapanese(MHL7Factory& factory, MDBImageManagerJapanese* database,
...
void
MLDispatchReportMgrJapanese::logHL7Stream(const char* txt, int len)
{
  if (mAnalysisMode) {
    ...
    sprintf(logFile, "%s/%d.hl7", storageDir, ++gMsgNumber);
    {
      ...
      logClient.log(MLogClient::MLOG_VERBOSE,
                    "",
                    "MLDispatchReportMgrJapanese::logHL7Stream", __LINE__, x);
    }
...
// virtual
int
MLDispatchReportMgrJapanese::acceptADT(MHL7Msg& message, const MString& event)
{
  MLogClient logClient;
  logClient.log(MLogClient::MLOG_CONVERSATION,
		"",
		"MLDispatchReportMgrJapanese::acceptADT",
		__LINE__,
...
// virtual
int
MLDispatchReportMgrJapanese::acceptORM(MHL7Msg& message, const MString& event)
{
  MLogClient logClient;
  logClient.log(MLogClient::MLOG_CONVERSATION,
		"",
		"MLDispatchReportMgrJapanese::acceptORM",
		__LINE__,
...
//virtual
int
MLDispatchReportMgrJapanese::acceptXXX(MHL7Msg& message, const MString& event)
{
  MLogClient logClient;
  logClient.log(MLogClient::MLOG_CONVERSATION,
		"",
		"MLDispatchReportMgrJapanese::acceptXXX",
		__LINE__,
...
void
MLDispatchReportMgrJapanese::processInfo(const MString& event, const MPatient& patient,
			 const MVisit& visit)
{
  MLogClient logClient;
  logClient.log(MLogClient::MLOG_VERBOSE,
		"",
		"MLDispatchReportMgrJapanese::processInfo",
		__LINE__,
		MString("Processing ADT event ")+event);

  cout << "Report Manager processing ADT event: " << event << endl;
  if (event == "A01") 
...
void
MLDispatchReportMgrJapanese::clearExistingMessages()
{
  MFileOperations f;
  ...
  logClient.log(MLogClient::MLOG_VERBOSE,
		"",
		"MLDispatchReportMgrJapanese::clearMessages", __LINE__, txt);
