...
MLStorageCommitment::MLStorageCommitment(const MString& logDir,
...
bool
MLStorageCommitment::supportsSCRole(DUL_SC_ROLE role)
{
  if (role == DUL_SC_ROLE_DEFAULT)
    return true;
  if (role == DUL_SC_ROLE_SCU)
    return true;
					  ...
					  MString& directoryName)
{
  ...
  MString fullPath = mStorageDir + "/" + apTitle;
