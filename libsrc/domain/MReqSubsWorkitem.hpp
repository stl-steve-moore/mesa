...
// $Id: MReqSubsWorkitem.hpp,v 1.2 2006/06/29 16:08:31 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.2 $ $Date: 2006/06/29 16:08:31 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MReqSubsWorkitem.hpp
//
//  = AUTHOR
//	David Maffitt
//
...
#ifndef MReqSubsWorkitemISIN
#define MReqSubsWorkitemISIN
...
class MReqSubsWorkitem;
typedef vector < MReqSubsWorkitem > MReqSubsWorkitemVector;

class MReqSubsWorkitem : public MDomainObject
// = TITLE
///	a domain object which corresponds to Requested Subsequent Workitem code sequence items.
//
// = DESCRIPTION
...
public:
  ...
  MReqSubsWorkitem();
  ///< Default constructor
  MReqSubsWorkitem(const MReqSubsWorkitem& cpy);
  virtual ~MReqSubsWorkitem();
  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
             to print the current state of MReqSubsWorkitem */
  ...
  MReqSubsWorkitem( MString& s, const MString& key);
...
inline ostream& operator<< (ostream& s, const MReqSubsWorkitem& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MReqSubsWorkitem& c) {
  c.streamIn(s);
  return s;
}
