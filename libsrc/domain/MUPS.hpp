...
// $Id: MUPS.hpp,v 1.2 2008/03/19 20:43:01 smm Exp $ $Author: smm $ $Revision: 1.2 $ $Date: 2008/03/19 20:43:01 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MUPS.hpp
...
class MUPS : public MDomainObject
// = TITLE
...
public:
  ...
  /**<\brief This method is used in conjunction with the streaming operator <<
  	     to print the current state of MUPS. */

  virtual void streamIn(istream& s);
  /**<\brief This method is used in conjunction with the streaming operator >>
  	     to read the current state of MUPS. */

  MUPS(const MString& studyInstanceUID,
  ...
  //SOP Common Module.  Set attributes.
  void specificCharacterSet(const MString& s);
  ...
  void scheduledProcedureStepPriority(const MString& s);
  void scheduledProcedureStepModDateTime(const MString& s);
...
private:
  // SOP Common Module
  MString mSpecificCharacterSet;
  MString mSOPClassUID;
  MString mSOPInstanceUID;

  // Unified Procedure Step Scheduled Procedure Information
  MString mSPSPriority;
  MString mSPSModificationDateTime;
...
typedef vector< MUPS > MUPSVector;

inline ostream& operator<< (ostream& s, const MUPS& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MUPS& c) {
  c.streamIn(s);
  return s;
}
