...
// $Id: MDBOrderFillerJapanese.hpp,v 1.4 2008/03/02 03:55:23 smm Exp $ $Author: smm $ $Revision: 1.4 $ $Date: 2008/03/02 03:55:23 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MDBOrderFillerJapanese.hpp
//
//  = AUTHOR
//	Phil DiCorpo
//
...
//
//  = DATE RELEASED
//	$Date: 2008/03/02 03:55:23 $
//
//  = COMMENTS

#ifndef MDBOrderFillerJapaneseISIN
#define MDBOrderFillerJapaneseISIN
...
class MDBOrderFillerJapanese : public MDBOrderFillerBase
// = TITLE
...
public:
  ...
  MDBOrderFillerJapanese(const MDBOrderFillerJapanese& cpy);

  virtual ~MDBOrderFillerJapanese();

  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
   to print the current state of MDBOrderFillerJapanese. */

  virtual void streamIn(istream& s);
  
  // = Class specific methods.  Methods return 0 on success and -1 on failure.

  MDBOrderFillerJapanese(const MString& databaseName);
  ...
  ///< This method should be used in conjunction with the default constructor.
  /** It opens the database named by <{databaseName}>. */

  int admitRegisterPatient(const MPatient& patient, const MVisit& visit);
  // A patient has been admitted as an in-patient or registered as an
  ...
  // Invoke this method to update the registration information of an
  // existing patient.  In the argument list,
  // <{patient}> contains the attributes to be udpated.

  int updateADTInfo(const MPatient& patient, const MVisit& visit);
  // Invoke this method to update the registration and visit information of
  // an existing patient.  In the argument list,
  // <{patient}> and <{visit}> contain the attributes to be udpated
  // in the database.

  int transferPatient(const MVisit& visit);
  // Patient has been transferred.  Update our records with new location
  ...
  // An order is placed if patient record exists.  Enter the order information
  ...
  // This method is called to update the Placer Order Number.  This typically
  ...
  // Update all fields in the Order database corresponding to the order
  // passed in <{order}>.  This method requires that a patient record
  // exist in the database before the update is performed.
  ...
  // Retrieve a Placer Order from the database.  Search on the patient ID
  // in the <{patient}> and the Placer Order Number in <{order}>.  Retrieve
  // the order and write it into the caller's <{order}> variable.

  int getOrder(MPatient& patient, MFillerOrder& order);
  // Retrieve a Filler Order from the database.  Search on the patient ID
  ...
  // Query for Modality Worklist items.  The caller's <{MWL}> variable provides
  ...
  // Query for Unified Worklist items.  The caller's <{UPS}> variable provides
  // the search criteria.  For each record found in the database, invoke
  // <{selectCallback}> method on the caller's <{client}> object.

  int addToVector(const MMWL& mwl);
  // This method is public for use in callback functions.  It add one
  // MMWL record to an internal vector.
  int addToVector(const MUPS& ups);
  // This method is public for use in callback functions.  It add one
  // MUPS record to an internal vector.
...
private:
  ...
  int mapFromEUCToISO(MMWL& mwl);
  int mapFromISOToEUC(MOrder& o);

};

inline ostream& operator<< (ostream& s, const MDBOrderFillerJapanese& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MDBOrderFillerJapanese& c) {
  c.streamIn(s);
  return s;
}
