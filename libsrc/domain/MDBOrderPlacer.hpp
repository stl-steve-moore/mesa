...
// $Id: MDBOrderPlacer.hpp,v 1.20 2006/06/29 16:08:30 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.20 $ $Date: 2006/06/29 16:08:30 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MDBOrderPlacer.hpp
...
#ifndef MDBOrderPlacerISIN
#define MDBOrderPlacerISIN
...
public:
  ...
  MDBOrderPlacer(const MDBOrderPlacer& cpy);

  virtual ~MDBOrderPlacer();
  ...
  MDBOrderPlacer(const MString& databaseName);
...
inline ostream& operator<< (ostream& s, const MDBOrderPlacer& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MDBOrderPlacer& c) {
  c.streamIn(s);
  return s;
}
