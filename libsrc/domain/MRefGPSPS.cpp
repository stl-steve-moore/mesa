//
//        Copyright (C) 1999, 2000, HIMSS, RSNA and Washington University
//
//        The MESA test tools software and supporting documentation were
//        developed for the Integrating the Healthcare Enterprise (IHE)
//        initiative Year 1 (1999-2000), under the sponsorship of the
//        Healthcare Information and Management Systems Society (HIMSS)
//        and the Radiological Society of North America (RSNA) by:
//                Electronic Radiology Laboratory
//                Mallinckrodt Institute of Radiology
//                Washington University School of Medicine
//                510 S. Kingshighway Blvd.
//                St. Louis, MO 63110
//        
//        THIS SOFTWARE IS MADE AVAILABLE, AS IS, AND NEITHER HIMSS, RSNA NOR
//        WASHINGTON UNIVERSITY MAKE ANY WARRANTY ABOUT THE SOFTWARE, ITS
//        PERFORMANCE, ITS MERCHANTABILITY OR FITNESS FOR ANY PARTICULAR
//        USE, FREEDOM FROM ANY DEFECTS OR COMPUTER DISEASES OR ITS CONFORMITY 
//        TO ANY SPECIFICATION. THE ENTIRE RISK AS TO QUALITY AND PERFORMANCE OF
//        THE SOFTWARE IS WITH THE USER.
//
//        Copyright of the software and supporting documentation is
//        jointly owned by HIMSS, RSNA and Washington University, and free
//        access is hereby granted as a license to use this software, copy
//        this software and prepare derivative works based upon this software.
//        However, any distribution of this software source code or supporting
//        documentation or derivative works (source code and supporting
//        documentation) must include the three paragraphs of this copyright
//        notice.

#include "MESA.hpp"
#include "MRefGPSPS.hpp"

MRefGPSPS::MRefGPSPS()
{
  tableName("refGPSPS");
  insert("sopclassuid");
  insert("sopinstanceuid");
  insert("transactionuid");
  insert("workitemkey");
}

MRefGPSPS::MRefGPSPS(const MRefGPSPS& cpy) :
  mSOPClassUID(cpy.mSOPClassUID),
  mSOPInstanceUID(cpy.mSOPInstanceUID),
  mTransactionUID(cpy.mTransactionUID),
  mWorkitemkey(cpy.mWorkitemkey)
{
  tableName("refGPSPS");
  insert("sopclassuid", cpy.mSOPClassUID);
  insert("sopinstanceuid", cpy.mSOPInstanceUID);
  insert("transactionuid", cpy.mTransactionUID);
  insert("workitemkey", cpy.mWorkitemkey);
}

MRefGPSPS::~MRefGPSPS()
{
}

void
MRefGPSPS::printOn(ostream& s) const
{
  s << "MRefGPSPS:"
    << mSOPClassUID  << ":"
    << mSOPInstanceUID << ":"
    << mTransactionUID << ":"
    << mWorkitemkey << "\n";
}

MRefGPSPS::MRefGPSPS(
           const MString& sopclassuid,
           const MString& sopinstanceuid, const MString& transactionuid,
           const MString& workitemkey
           ) :
  mSOPClassUID(sopclassuid),
  mSOPInstanceUID(sopinstanceuid),
  mTransactionUID(transactionuid),
  mWorkitemkey(workitemkey)
{
  tableName("refGPSPS");
  insert("sopclassuid", sopclassuid);
  insert("sopinstanceuid", sopinstanceuid);
  insert("transactionuid", transactionuid);
  insert("workitemkey", workitemkey);
}

MString
MRefGPSPS::sopClassUID() const
{
  return mSOPClassUID;
}

MString
MRefGPSPS::sopInstanceUID() const
{
  return mSOPInstanceUID;
}

MString
MRefGPSPS::transactionUID() const
{
  return mTransactionUID;
}

MString
MRefGPSPS::workitemkey() const
{
  return mWorkitemkey;
}

void
MRefGPSPS::sopClassUID(const MString& s)
{
  mSOPClassUID = s;
  remove("sopclassuid");
  insert("sopclassuid", s);
}

void
MRefGPSPS::sopInstanceUID(const MString& s)
{
  mSOPInstanceUID = s;
  remove("sopinstanceuid");
  insert("sopinstanceuid", s);
}

void
MRefGPSPS::transactionUID(const MString& s)
{
  mTransactionUID = s;
  remove("transactionuid");
  insert("transactionuid", s);
}

void
MRefGPSPS::workitemkey(const MString& s)
{
  mWorkitemkey = s;
  remove("workitemkey");
  insert("workitemkey", s);
}

void
MRefGPSPS::streamIn(istream& s)
{
  //s >> this->member;
}

void MRefGPSPS::import(const MDomainObject& o)
{
  sopClassUID(o.value("sopclassuid"));
  sopInstanceUID(o.value("sopinstanceuid"));
  transactionUID(o.value("transactionuid"));
  workitemkey(o.value("workitemkey"));
}
