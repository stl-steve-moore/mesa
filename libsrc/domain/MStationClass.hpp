...
// $Id: MStationClass.hpp,v 1.2 2006/06/29 16:08:31 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.2 $ $Date: 2006/06/29 16:08:31 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MStationClass.hpp
...
#ifndef MStationClassISIN
#define MStationClassISIN
...
class MStationClass;
typedef vector < MStationClass > MStationClassVector;

class MStationClass : public MDomainObject
// = TITLE
///	a domain object which corresponds to a coded entry according to DICOM definitions.
//
// = DESCRIPTION
...
public:
  ...
  MStationClass();
  ///< Default constructor
  MStationClass(const MStationClass& cpy);
  virtual ~MStationClass();
  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
  	     to print the current state of MStationClass */
  ...
  MStationClass( MString& s, const MString& key);
...
inline ostream& operator<< (ostream& s, const MStationClass& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MStationClass& c) {
  c.streamIn(s);
  return s;
}
