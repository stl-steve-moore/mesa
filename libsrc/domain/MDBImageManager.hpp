...
// $Id: MDBImageManager.hpp,v 1.23 2006/06/29 16:08:29 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.23 $ $Date: 2006/06/29 16:08:29 $ $State: Exp $
//
// ====================
//
//  = FILENAME
...
#ifndef MDBImageManagerISIN
#define MDBImageManagerISIN
...
public:
  ...
  MDBImageManager(const MDBImageManager& cpy);

  virtual ~MDBImageManager();

  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
  to print the current state of MDBImageManager. */

  virtual void streamIn(istream& s);
  /**<\brief This method is used in conjunction with the streaming operator >>
   to read the current state of MDBImageManager. */

  // = Class specific methods.  All methods return 0 on success and -1 on
  // failure unless otherwise noted.

  MDBImageManager(const string& databaseName);
  ...
  virtual int initialize(const MString& databaseName);
  ///< Initialize the Image Manager.  Open the database named in <{databaseName}> and prepare to run.

  virtual int admitRegisterPatient(const MPatient& patient, const MVisit& visit);
  ...
  virtual int preRegisterPatient(const MPatient& patient, const MVisit& visit);
  ///< A patient has been pre registered.
  /**< The <{visit}> argument defines the registration mode.
   Update our tables accordingly. */

  virtual int updateADTInfo(const MPatient& patient);
  ...
  virtual int updateADTInfo(const MPatient& patient, const MVisit& visit);
  ...
  virtual int transferPatient(const MVisit& visit);
  ///< Patient has been transferred.  
  /**< Update our records with new location
   described in the <{visit}> argument. */

  virtual int mergePatient(const MPatient& patient);

  virtual int enterOrder(const MPatient& patient, const MPlacerOrder& order);
  ...
  virtual int queryDICOMPatientRoot(const MPatient& patient, const MStudy& study,
  ...
  virtual int queryDICOMStudyRoot(const MPatientStudy& patientStudy,
			  const MSeries& series,
			  const MSOPInstance& sopInstance,
  ...
  virtual int selectSOPInstance(const MSOPInstance& sopInstance,
  ...
  virtual int selectSOPInstance(const MSOPInstance& pattern,
			MSOPInstance& target);
  ...
  virtual int selectSOPInstance(MSOPInstanceVector& v);
  ///< Query for all SOP Instances only.  
  /**< The method queries for all SOP Instances and fills the vector <{v}> with the results. */

  virtual int enterStorageCommitmentItem(const MStorageCommitItem& item);
  ///< Insert one Store Commitment Item in Storage Commitment table.

  virtual int selectStorageCommitItem(const MStorageCommitItem& item,
  ...
  virtual int selectStorageCommitItem(const MStorageCommitItem& pattern,
			      MStorageCommitItem& target);
  ///< Query for a single Storage Commitment item.  
  /**< The attributes of the variable
   <{pattern}> are used as the criteria for the select.  If a single record
   matches the select criteria, it is written into the caller's <{target}>
   variable. */

  virtual int storageCommitmentStatus(MStorageCommitItem& item);
  ///< This method is a placeholder.  It is not implemented.

  virtual int selectDICOMApp(const MString& aeTitle, MDICOMApp& app);
  ...
  virtual int addToVector(const MPatient& patient);
  /**<\brief This method is only public so that it may be invoked from a callback.
   During the select process in the methods above, add one patient record
   to the internal vector of patient records. */

  virtual int addToVector(const MStudy& study);
  /**<\brief This method is only public so that it may be invoked from a callback.
   During the select process in the methods above, add one study record
   to the internal vector of study records. */

  virtual int addToVector(const MSeries& series);
  /**<\brief This method is only public so that it may be invoked from a callback.
   During the select process in the methods above, add one series record
   to the internal vector of series records. */

  virtual int queryWorkOrder(MDBIMClient& client, int status = -1);

  virtual int deleteWorkOrder(const MString& orderNumber);

  virtual int updateWorkOrderStatus(const MWorkOrder& workOrder, const MString& status);

  virtual int setWorkOrderComplete(const MWorkOrder& workOrder, const MString& status);
...
private:
  ...
  void updateRecord(const MStudy& study);
...
inline ostream& operator<< (ostream& s, const MDBImageManager& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MDBImageManager& c) {
  c.streamIn(s);
  return s;
}
