...
// $Id: MDBXRefMgr.hpp,v 1.3 2006/06/29 16:08:30 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.3 $ $Date: 2006/06/29 16:08:30 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MDBXRefMgr.hpp
...
#ifndef MDBXRefMgrISIN
#define MDBXRefMgrISIN
...
public:
  ...
  MDBXRefMgr(const MDBXRefMgr& cpy);

  virtual ~MDBXRefMgr();

  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
  	     to print the current state of MDBXRefMgr. */

  virtual void streamIn(istream& s);
  
  // = Class specific methods.  Methods return 0 on success and -1 on failure.

  MDBXRefMgr(const MString& databaseName);
...
inline ostream& operator<< (ostream& s, const MDBXRefMgr& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MDBXRefMgr& c) {
  c.streamIn(s);
  return s;
}
