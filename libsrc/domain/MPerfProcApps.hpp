...
// $Id: MPerfProcApps.hpp,v 1.2 2006/06/29 16:08:31 bhasselfeld Exp $ $Author: bhasselfeld $ $Revision: 1.2 $ $Date: 2006/06/29 16:08:31 $ $State: Exp $
//
// ====================
//  = FILENAME
//	MPerfProcApps.hpp
...
#ifndef MPerfProcAppsISIN
#define MPerfProcAppsISIN
...
class MPerfProcApps;
typedef vector < MPerfProcApps > MPerfProcAppsVector;

class MPerfProcApps : public MDomainObject
// = TITLE
/**\brief	a domain object which corresponds to Performed Processing Applications
	      	code sequence items. */
//
// = DESCRIPTION
...
public:
  ...
  MPerfProcApps();
  ///< Default constructor
  MPerfProcApps(const MPerfProcApps& cpy);
  virtual ~MPerfProcApps();
  virtual void printOn(ostream& s) const;
  /**<\brief This method is used in conjunction with the streaming operator <<
  	     to print the current state of MPerfProcApps */
  ...
  MPerfProcApps( MString& s, const MString& key);
...
inline ostream& operator<< (ostream& s, const MPerfProcApps& c) {
	  c.printOn(s);
	  return s;
};

inline istream& operator >> (istream& s, MPerfProcApps& c) {
  c.streamIn(s);
  return s;
}
