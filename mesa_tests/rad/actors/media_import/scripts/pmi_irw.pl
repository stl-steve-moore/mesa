...
# Runs the Portable Media Importer tests interactively
...
#
#  if ($modality eq "MR") {
#    pmi::local_scheduling_mr($logLevel);
#  } elsif ($modality eq "RT") {
#    pmi::local_scheduling_rt($logLevel);
#  } else {
...
sub processTransaction {
  ...
  if ($trans eq "RAD-1") {
    ...
    $rtnValue = pmi_transactions::processTransaction1($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-2") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransaction2($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-5") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransaction5($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "2") {
    shift (@tokens); shift (@tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction4a($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4c") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction4c($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4aa") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction4aa($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4ax") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction4ax($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4b") {
    shift (@tokens); shift (@tokens);
    $rtnValue = processTransaction4b(@tokens);
  } elsif ($trans eq "4") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction4($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "5") {
    shift (@tokens); shift (@tokens);
    $rtnValue = processTransaction5($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "5a") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction5a($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "6") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction6(@tokens);
  } elsif ($trans eq "7") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction7(@tokens);
  } elsif ($trans eq "8") {
    shift (@tokens); shift (@tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransaction12($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "13") {
    shift (@tokens); shift (@tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction37($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "42") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction42($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "48") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::processTransaction48($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-X") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransactionX($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-Y") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransactionY($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-Z") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi_transactions::processTransactionZ($logLevel, $selfTest, @tokens);
  } else {
...
sub processMESAInternal {
  ...
  if ($trans eq "RAD-SCHEDULE") {
    shift (@tokens); shift(@tokens);
    #$rtnValue = pmi::processInternalSchedulingRequest($logLevel, $selfTest, @tokens);
    ...
    shift (@tokens); shift (@tokens);
    #$rtnValue = pmi::generateSOPInstances($logLevel, $selfTest, @tokens);
...
sub processMessage {
  ...
  if ($trans eq "RAD-SCHEDULE") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pmi::announceSchedulingParameters($logLevel, $selfTest, @tokens);
  } else {
    die "Unable to process command <$cmd>\n";
  }
  return $rtnValue;
...
sub testConfigurationVariables {
  die "No test PMI Host HL7 " if ! $testPMIHostHL7;
  die "No test PMI Port HL7 " if ! $testPMIPortHL7;
  die "No PMI Port HL7" if ! $mesaPMIPortHL7;
  die "No MESA_ORD_PLC_PORT_HL7" if ! $mesaOrderPlacerPortHL7;
  die "No MESA_IMG_MGR_PORT_HL7" if ! $mesaIMPortHL7;
  die "No MESA_IMG_MGR_PORT_DCM" if ! $mesaIMPortDICOM;
  die "No MESA IM AE MPPS" if ! $mesaIMAEMPPS;
  die "No MESA IM AE CSTORE" if ! $mesaIMAECStore;
  die "No MESA ORD FIL PORT HLT" if ! $mesaOFPortHL7;
  die "No MESA_OF_DICOM_PORT" if ! $mesaOFPortDICOM;
  die "No MESA_ORD_FIL_HOST" if ! $mesaOFHost;
  die "No MESA_ORD_FIL_AE_MWL" if ! $mesaOFAEMWL;
}
...
if (pmi::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in pmi_test.cfg\n";
  exit 1;
}

$testPMIHostHL7 = $varnames{"TEST_HL7_HOST"};
$testPMIPortHL7 = $varnames{"TEST_HL7_PORT"};
...
$mesaOFPortHL7        = $varnames{"MESA_ORD_FIL_PORT_HL7"};
$mesaOFPortDICOM      = $varnames{"MESA_ORD_FIL_PORT_DCM"};
$mesaOFHost           = $varnames{"MESA_ORD_FIL_HOST"};
$mesaOFAEMWL          = $varnames{"MESA_ORD_FIL_AE_MWL"};
