...
sub processTransaction4aa {
  ...
  pmi::update_object("$main::MESA_STORAGE/modality/$outputDir/mpps.crt",    $deltaNCreate);
  pmi::update_object("$main::MESA_STORAGE/modality/$outputDir/mpps.set",    $deltaNSet);
  pmi::update_object("$main::MESA_STORAGE/modality/$outputDir/mpps.status", $deltaNSet);
  return 0;
}
...
sub processTransactionX {
  my ($logLevel, $selfTest, $src, $dst, $event, $dir, $modalityAEMPPS) = @_;

  print "IHE RAD-X: \n";
  print "PMI actor is expected to send MPPS IN PROGRESS message (N-Create)\n";
  ...
  if ($selfTest == 1) {
    my $inputDir = "$main::MESA_STORAGE/modality/$dir/mpps";
    $status = mesa_xmit::send_MPPS_in_progress($logLevel, $inputDir, $modalityAEMPPS, $main::mesaIMAEMPPS, "localhost", $main::mesaIMPortDICOM);
    if ($status != 0) {
      print "pmi_transactions::processTransactionX could not send MPPS N-Create to MESA in self test mode\n";
      return 1;
    }
  }
  return 0;
...
sub processTransactionY {
  my ($logLevel, $selfTest, $src, $dst, $event, $dir, $modalityAEMPPS) = @_;

  print "IHE RAD-Y: \n";
  print "PMI actor is expected to send MPPS $event message (N-Set)\n";
  print "MPPS N-Create should go to MESA Image Manager ($main::mesaIMAEMPPS, $main::mesaIMPortDICOM)\n";
  print "Hit <ENTER> when ready (q to quit) --> ";
  my $x = <STDIN>;
  main::goodbye if ($x =~ /^q/);
  ...
  if ($selfTest == 1) {
    ...
    if ($status != 0) {
      print "pmi_transactions::processTransactionY could not send MPPS N-Set to MESA in self test mode\n";
      return 1;
    }
  }
  return 0;
...
# processTransactionZ
# This is C-Store
sub processTransactionZ {
  my ($logLevel, $selfTest, $src, $dst, $event, $dir, $modalityAECStore) = @_;

  print "IHE RAD-Z: \n";
  print "PMI actor is expected to send DIOCOM objects (C-Store)\n";
  ...
  if ($selfTest == 1) {
    ...
    if ($status != 0) {
      print "pmi_transactions::processTransactionZ could not C-Store objects to MESA in self test mode\n";
      return 1;
    }
  }
  return 0;
