...
sub find_candidate_files {
  my $tag = shift(@_);
  my $val = shift(@_);
  print LOG "Report Manager 653-j\n";
...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -r 653-j/sr_653_req.txt -o PREDECESSOR";
    $x .= " -p ../../msgs/sr/603-j/sr_603mr_one_ref.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 653-j/sr_653.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/603-j/sr_603mr_one_ref.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">653-j/grade_653.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 653-j/grade_653.txt \n";
