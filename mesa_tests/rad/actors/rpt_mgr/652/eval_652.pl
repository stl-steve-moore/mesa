...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -r 652/sr_652_req.txt -o PREDECESSOR";
    $x .= " -p ../../msgs/sr/602/sr_602ct_add.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 652/sr_652.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/602/sr_602ct_add.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">652/grade_652.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 652/grade_652.txt \n";
