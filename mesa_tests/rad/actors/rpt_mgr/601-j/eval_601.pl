...
sub find_candidate_files {
  my $tag = shift(@_);
  my $val = shift(@_);
  print LOG "Report Manager 601-j\n";
...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -r 601-j/sr_601_req.txt -o PREDECESSOR ";
    $x .= " -p ../../msgs/sr/601-j/sr_601cr_v.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 601-j/sr_601.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/601-j/sr_601cr_v.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">601-j/grade_601.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 601-j/grade_601.txt \n";
