...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -r 654/sr_654_req.txt -o PREDECESSOR";
    $x .= " -p ../../msgs/sr/604/sr_604ct_modified.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 654/sr_654.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/604/sr_604ct_modified.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">654/grade_654.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 654/grade_654.txt \n";
