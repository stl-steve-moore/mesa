...
# Assume we include performed protocol code sequence in MPPS
my $protocolCodeFlag = 1;
if (scalar(@ARGV) == 12) {
 $protocolCodeFlag = $ARGV[11];
}
...
if (! (-e "mwl/mwlquery-j.txt") ) {
  print "The file mwl/mwlquery.txt does not exist.  Installation error. \n";
  exit 1;
}

$x = "$MESA_TARGET/bin/dcm_create_object -i mwl/mwlquery-j.txt mwl/mwlquery.dcm";
print "$x \n";
print `$x`;
if ($? != 0) {
