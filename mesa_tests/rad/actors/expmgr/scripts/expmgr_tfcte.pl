...
# Runs the Export Manager scripts interactively
...
sub processTransaction {
  ...
  if ($trans eq "RAD-50") {
    ...
    #goodbye if ($x =~ /^q/);
    #return 0 if ($x =~ /^s/);
    $rtnValue = expmgr::processTransaction50($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-51") {
    shift (@tokens); shift (@tokens);
    $rtnValue = expmgr::processTransaction51($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-52") {
    shift (@tokens); shift (@tokens);
    $rtnValue = expmgr::processTransaction52($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-53") {
    shift (@tokens); shift (@tokens);
    $rtnValue = expmgr::processTransaction53($logLevel, $selfTest, @tokens);
  } else {
...
%varnames = mesa::get_config_params("expmgr_test.cfg");
if (expmgr::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in expmgr_test.cfg\n";
  exit 1;
}
...
$expMgrHost         = $varnames{"TEST_EXPMGR_HOST"};
$expMgrPort         = $varnames{"TEST_EXPMGR_PORT"};
$expMgrAE           = $varnames{"TEST_EXPMGR_AE"};

testVarValues($mesaExpMgrPortDICOM, $mesaExpMgrAE, $mesaExpRCRPortDICOM, $mesaExpRCRAE, $expMgrHost, $expMgrPort, $expMgrAE);
