...
sub x_313_3 {
 print LOG "\nCTX: Evidence Creator: 313.3 \n";
 print LOG "CTX: Dump Encapsulated PDF to a file\n";

 my ($logLevel, $f) = @_;

 my $x = "$MESA_TARGET/bin/dcm_dump_element 0042 0011 $f 313/313.pdf";
 `$x`;
 if ($?) {
...
$logLevel = $ARGV[0];
open LOG, ">313/grade_313.txt" or die "Could not open 313/grade_313.txt";
my $mesaVersion = mesa_get::getMESAVersion();
my ($x, $date, $timeToMinute, $timeToSec) = mesa_get::getDateTime(0);
print LOG "CTX: Evidence Creator test 313\n";
...
if ($fileName eq "") {
  ...
  $diff += x_313_2($logLevel, $fileName);
  $diff += x_313_3($logLevel, $fileName);
}

print LOG "\nTotal Differences: $diff \n";
print "\nTotal Differences: $diff \n";

print "Logs stored in 313/grade_313.txt \n";
