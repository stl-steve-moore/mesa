...
# Runs the Evidence Creator tests for the Fusion profile.
...
sub processTransaction {
  ...
  if ($trans eq "RAD-1") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction4a($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4c") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction4c($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4aa") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction4aa($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4ax") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction4ax($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4b") {
    shift (@tokens); shift (@tokens);
    $rtnValue = processTransaction4b(@tokens);
  } elsif ($trans eq "4") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction4($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "5") {
    shift (@tokens); shift (@tokens);
    $rtnValue = processTransaction5($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "5a") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction5a($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "6") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction6(@tokens);
  } elsif ($trans eq "7") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction7(@tokens);
  } elsif ($trans eq "RAD-8") {
    shift (@tokens); shift (@tokens);
    $rtnValue = evdcrt_transactions::processTransaction8($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-10") {
    shift (@tokens); shift (@tokens);
    $rtnValue = evdcrt_transactions::processTransaction10($logLevel, $selfTest, @tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction37($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "42") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction42($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "48") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imp::processTransaction48($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-56") {
    shift (@tokens); shift (@tokens);
    $rtnValue = evdcrt_transactions::processTransaction56($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "RAD-57") {
    shift (@tokens); shift (@tokens);
    $rtnValue = evdcrt_transactions::processTransaction57($logLevel, $selfTest, @tokens);
...
sub processMESAInternal {
  ...
  if ($trans eq "STORAGE-COMMITMENT") {
    shift (@tokens); shift (@tokens);
	...
	$paramsFile, $superimposed, $underlying, $dst);
  } elsif ($trans eq "BLENDING-SPATIAL") {
    shift (@tokens); shift (@tokens);
    my ($verb, $paramsFile, $superimposed, $underlying, $spatial, $dst) = @tokens;
    $paramsFile = "../common/" . $paramsFile;
    $superimposed = $main::MESA_STORAGE . "/" . $superimposed;
    $underlying = $main::MESA_STORAGE . "/" . $underlying;
    $spatial = $main::MESA_STORAGE . "/" . $spatial;
    $dst = $main::MESA_STORAGE . "/" . $dst;
    $rtnValue = mesa_dicom::create_BLENDING_SPATIAL($logLevel, $selfTest,
	$paramsFile, $superimposed, $underlying, $spatial, $dst);
  } else {
    die "Unable to process command <$cmd>";
  }
...
#    localScheduling($cmd);
    ...
    if ($verb[1] ne "FUS") {
      die "This Importer script is for the FUS profile, not $verb[1]";
    }
...
sub testConfigurationVariables {
  die "No TEST_CSTORE_SCP_HOST"		if ! $testCStoreSCPHost;
  die "No TEST_CSTORE_SCP_PORT"		if ! $testCStoreSCPPort;
  die "No TEST_CSTORE_SCP_AE"		if ! $testCStoreSCPAE;
  die "No TEST_STORAGE_COMMIT_AE"	if ! $testAEStorageCommit;
  die "No TEST_STORAGE_COMMIT_HOST"	if ! $testStorageCommitHost;
  die "No TEST_STORAGE_COMMIT_PORT"	if ! $testStorageCommitPort;

#  die "No MESA_IMG_MGR_HL7_PORT"  if ! $mesaIMPortHL7;
  die "No MESA_IMGMGR_DICOM_PT"		if ! $mesaIMPortDICOM;
#  die "No MESA_IMG_MGR_AE_MPPS"   if ! $mesaIMAEMPPS;
  die "No MESA_IMG_MGR_AE_CSTORE" if ! $mesaIMAECStore;
  die "No MESA_IMG_MGR_AE_STORAGE_COMMIT" if ! $mesaIMAEStorageCommit;
  die "No MESA_OF_HOST"		  if ! $mesaOFHost;
  die "No MESA_OF_DICOM_PORT"	  if ! $mesaOFPortDICOM;
  die "No MESA_OF_AE_MWL"	  if ! $mesaOFAEMWL;
  die "No MESA_OF_HL7_PORT"	  if ! $mesaOFPortHL7;
}
...
%varnames = mesa_get::get_config_params("evdcrt_test.cfg");
if (evdcrt::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in imp_test.cfg\n";
  exit 1;
}

$testCStoreSCPHost	= $varnames{"TEST_CSTORE_SCP_HOST"};
$testCStoreSCPPort	= $varnames{"TEST_CSTORE_SCP_PORT"};
$testCStoreSCPAE	= $varnames{"TEST_CSTORE_SCP_AE"};
$testAEStorageCommit	= $varnames{"TEST_STORAGE_COMMIT_AE"};
$testStorageCommitHost	= $varnames{"TEST_STORAGE_COMMIT_HOST"};
$testStorageCommitPort	= $varnames{"TEST_STORAGE_COMMIT_PORT"};

#$mesaIMPortHL7        = $varnames{"MESA_IMG_MGR_HL7_PORT"};
$mesaIMPortDICOM      = $varnames{"MESA_IMGMGR_DICOM_PT"};
#$mesaIMAEMPPS         = $varnames{"MESA_IMG_MGR_AE_MPPS"};
$mesaIMAECStore		= $varnames{"MESA_IMG_MGR_AE_CSTORE"};
$mesaIMAEStorageCommit	= $varnames{"MESA_IMG_MGR_AE_STORAGE_COMMIT"};
