...
sub makeQuery {
  my $x = "$MESA_TARGET/bin/dcm_create_object -i 50401/query.txt 50401/query.dcm";
  `$x`;
...
sub makeDirectories {
  mesa_utility::delete_directory(0, "50401/test");
  mesa_utility::create_directory(0, "50401/test");
  mesa_utility::delete_directory(0, "50401/mesa");
  mesa_utility::create_directory(0, "50401/mesa");
}

sub sendQuery {
  $x = mesa_xmit::sendCFind($logLevel, "STUDY",
	"50401/query.dcm", $imCFindAE, $imCFindHost, $imCFindPort, "VISION1", "50401/test");

  die "Could not send C-Find to IM under test\n" if ($x != 0);

  $x = mesa_xmit::sendCFind($logLevel, "STUDY",
	"50401/query.dcm", $imCFindAE, "localhost", $mesaImgMgrPortDICOM, "VISION1", "50401/mesa");
