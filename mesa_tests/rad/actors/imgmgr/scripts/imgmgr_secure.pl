...
sub transaction3_OF {
  ...
  if ($event ne "O02") {
    ...
    print "This script expects the event to be O02. This must be an\n";
...
sub processTransaction {
  ...
  if ($trans eq "1") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction4aSecure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4b") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction4bSecure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "4") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction4Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "5") {
    shift (@tokens); shift (@tokens);
    processTransaction5(@tokens);
  } elsif ($trans eq "6") {
    shift (@tokens); shift (@tokens);
    imgmgr::processTransaction6Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "7") {
    shift (@tokens); shift (@tokens);
    imgmgr::processTransaction7Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "8") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction8Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "10") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction10Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "12") {
    shift (@tokens); shift (@tokens);
    imgmgr::processTransaction12Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "13") {
    shift (@tokens); shift (@tokens);
    processTransaction13(@tokens);
  } elsif ($trans eq "14") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransaction14Secure($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "16") {
    shift (@tokens); shift (@tokens);
...
sub processMESAInternal {
  ...
  if ($trans eq "RAD-SCHEDULE") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = mesa::generateSOPInstancesSecure($logLevel, $selfTest, "IM", "MESAMWL", $mesaOFHost, $mesaOFPortDICOM, @tokens);
...
sub processMessage {  my $cmd = shift(@_);
  my $logLevel = shift(@_);
  my $selfTest = shift(@_);
  my @tokens = split /\s+/, $cmd;  my $verb = $tokens[0];
  my $trans= $tokens[1];
  if ($trans eq "RAD-SCHEDULE") {
...
%varnames = mesa::get_config_params("imgmgr_secure.cfg");
if (imgmgr::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in imgmgr_secure.cfg\n";
  exit 1;
}
...
testVarValues($imCommitAE, $mppsPort,
 ...
 $mesaModPortDICOM, $mesaImgMgrPortHL7);
