...
$mwlQueryExe = "$MESA_TARGET/bin/mwlquery_secure -C $C -K $K -P $P -R $R  -a $modalityAE -c $mwlAE -d pid.txt -f mwl/mwlquery.dcm -o mwl/results $mwlHost $mwlPort";
print MWLOUTPUT `$mwlQueryExe`;
if ($?) {
