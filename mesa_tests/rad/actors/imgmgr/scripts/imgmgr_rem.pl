...
sub processTransaction {
  ...
  } elsif ($trans eq "RAD-62") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr_transactions::processTransaction62($logLevel, $selfTest, @tokens);
  } else {
...
#    localScheduling($cmd);
    ...
    if ($verb[1] ne "REM") {
      die "This Image Manager script is for the REM profile, not $verb[1]";
    }
