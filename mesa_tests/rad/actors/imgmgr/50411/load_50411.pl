...
my $status = mesa_xmit::sendCStoreDirectory(
	$logLevel, "50411/delta.txt", "$MESA_STORAGE/EYECARE/PDF1", "VISION1",
	$imCStoreAE, $imCStoreHost, $imCStorePort, 0);

die "Could not send Encapsulated PDF to IM under test\n" if ($status != 0);

$status = mesa_xmit::sendCStoreDirectory(
	$logLevel, "50411/delta.txt", "$MESA_STORAGE/EYECARE/PDF1", "VISION1",
	"MESA-IM", "localhost", $mesaImgMgrPortDICOM, 0);

die "Could not send Encapsulated PDF to MESA IM\n" if ($status != 0);
