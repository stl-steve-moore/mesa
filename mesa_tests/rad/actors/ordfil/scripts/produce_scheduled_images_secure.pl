...
`$MESA_TARGET/bin/dcm_create_object -i mwl/mwlquery.txt mwl/mwlquery.dcm`;
if ($? != 0) {
...
$x = "$MESA_TARGET/bin/mwlquery_secure -C $C -K $K -P $P -R $R -a $modalityAE -c $mwlAE -d pid.txt -f mwl/mwlquery.dcm -o mwl/results $mwlHost $mwlPort";
print MWLOUTPUT "$x\n";
print MWLOUTPUT `$x`;
if ($?) {
