...
#  my $inputORM = "$MESA_STORAGE/ordplc/1001.hl7";
  my $inputORM = "$MESA_STORAGE/$extraOrder";
...
sub processTransaction {
  ...
  if ($trans eq "1") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction5a($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "6") {
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction6(@tokens);
  } elsif ($trans eq "7") {
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction7(@tokens);
  } elsif ($trans eq "8") {
    shift (@tokens); shift (@tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction37($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "42") {
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction42($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "48") {
    shift (@tokens); shift (@tokens);
    $rtnValue = ordfil::processTransaction48($logLevel, $selfTest, @tokens);
  } else {
...
sub processMESAInternal {
  ...
  if ($trans eq "RAD-SCHEDULE") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = mesa::generateMPPSAbandonCoded($logLevel, $selfTest, $mwlAE, $mwlHost, $mwlPort, @tokens);
...
$gpppsSCPAE    = $varnames{"TEST_GPPPS_SCP_AE"};   die "<TEST_GPPPS_SCP_AE>"   if !$gpppsSCPAE;
$gpppsSCPHost  = $varnames{"TEST_GPPPS_SCP_HOST"}; die "<TEST_GPPPS_SCP_HOST>" if !$gpppsSCPHost;
$gpppsSCPPort  = $varnames{"TEST_GPPPS_SCP_PORT"}; die "<TEST_GPPPS_SCP_PORT>" if !$gpppsSCPPort;
...
$mesaGPPPSSCPPort = $mesaOFPortDICOM;	die "<MESA_ORD_FIL_PORT_DCM>" if !$mesaGPPPSSCPPort;
$mesaPPMPortDICOM = $varnames{"MESA_PPM_PORT"};	die "<MESA_PPM_PORT>" if !$mesaPPMPortDICOM;
