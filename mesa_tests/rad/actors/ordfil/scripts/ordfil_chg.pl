...
sub processTransaction35 {
  ...
  print "\nIHE Transaction 35: $pid $patientName \n";
  print "DSS/OF is expected to send HL7 message to MESA Charge Processor ($host $mesaCPPortHL7) for event $event \n";
  ...
  if ($selfTest == 1) {
    print "Looks like this is run directly for MESA testing; send order to MESA CP \n";
    $x = mesa::send_hl7("../../msgs", $msg, "localhost", $mesaCPPortHL7);
    mesa::xmit_error($msg) if ($x != 0);
  } else {
  }
  return 0;
...
sub processTransaction36 {
  ...
  if ($dst ne "OF") {
    print "IHE Transaction 36 from ($src) to ($dst) is not required for DSS/OF test\n";
    return 0;
  }
  ...
  print "\nMESA will now send BAR message ($msg) to your Order Filler ($ofHostHL7 $ofPortHL7) \n";
  print "Hit <ENTER> when ready (q to quit) --> ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
  $x = mesa::send_hl7_log($logLevel, "../../msgs", $msg, $ofHostHL7, $ofPortHL7);
...
$mesaCPPortHL7   = $varnames{"MESA_CHG_PRC_PORT_HL7"};
