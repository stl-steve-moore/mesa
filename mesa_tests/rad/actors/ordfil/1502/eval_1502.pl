...
sub x_1502_1 {
  print LOG "Order Filler 1502.1 \n";
  print LOG "Audit Record Messages \n";
  ordfil::clear_syslog_files();
  ordfil::extract_syslog_messages();
  my $xmlCount = ordfil::count_syslog_xml_files();
  if ($xmlCount < 9) {
    print LOG "We expect at least 9 audit messages from your Order Filler.\n";
    print LOG " We received $xmlCount messages \n";
    print LOG " That is a failure; we will evaluate the messages received.\n";
    $diff += 1;
  }
  $diff += ordfil::evaluate_all_xml_files();
}
