...
sub find_candidate_files {
  my $tag = shift(@_);
  my $val = shift(@_);
  print LOG "Report Creator 606-j\n";
...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -p ../../msgs/sr/606-j/sr_606mr.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 606-j/sr_606_mr.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/606-j/sr_606mr.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">606-j/grade_606_mr.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 606-j/grade_606_mr.txt \n";
