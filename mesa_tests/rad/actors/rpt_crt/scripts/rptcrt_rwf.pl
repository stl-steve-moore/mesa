...
sub processTransaction1 {
  ...
  if ($dst eq "OF") {
    ...
    print "\nMESA will send ADT message ($msg) for event $event to MESA $dst\n";
    $x = mesa::send_hl7_log($logLevel, "../../msgs", $msg, $mesaRptmgrHostHL7, $mesaRptmgrPortHL7);
    mesa::xmit_error($msg) if ($x != 0);
    return 0;
  } else {
...
sub processTransaction4 {
  ...
  print "MESA will send HL7 message to \n MESA Report Manager ($mesaRptmgrHostHL7 $mesaRptmgrPortHL7) for event $event \n";
  print " Placer Order Number is $placerOrderNumber \n";
  print "Hit <ENTER> when ready (q to quit) --> ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
...
sub processTransaction6 {
  ...
  print "\nIHE Transaction 6: \n";
  print "MESA will send MPPS message from dir ($inputDir) for event ($event) to MESA ($dst)\n";
  print "MPPS parameters for Report Manager: $mesaRptmgrAE $mesaRptmgrHostDICOM $mesaRptmgrPortDICOM \n";
  print "Hit <ENTER> when ready (q to quit) --> ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
...
sub processTransaction11 {
  ...
  print "\nIHE Transaction 11: Image Availability Query is not needed for Report Creator test\n";
  return 0;
}
...
sub processTransaction38a {
  ...
  print "\nIHE Transaction 38a: \n" .
  "MESA Report Creator will create a transaction UID in preparation for Transaction 38.\n";

  $resultDir = $outDir . "/mesa";

  # If not in the self test, then we can stop now.
...
sub processTransaction38 {
  ...
  print "\nIHE Transaction 38: \n" .
  "Your Report Creator ($test_rptcrt_AE) is expected to send a GPSPS NACTION\n" .
  "for the workitem to the MESA Report Mgr; event is $event\n" .
  "MESA DICOM parameters are: ($mesa_rptmgrRWL_AE, $host, $mesa_rptmgrRWL_Port)\n" .
  "Press <enter> when ready to continue or <q> to quit: ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
  ...
  if( $rtnVal == 1) {
    print "\nMESA Report Creator Failed to claim workitem.\n";
    return 1;
  }
  return 0;
}
...
#
#  print "\nIHE Transaction 41: \n" .
#  "Your Report Creator ($test_rptcrt_AE) is expected to send a GPSPS NACTION to\n" .
...
#  $dcmMsg = "../../msgs/" . $dcmMsg;
#
#  $mesa_spsSOPUID = mesa::getInstanceSOPUID($rwlDir . "/mesa/msg1_result.dcm");
#  chomp $mesa_spsSOPUID;
#  # print "mesa_spsSOPUID: " . $tmesaspsSOPUID . "\n";
#
#  mesa::delete_directory($logLevel, $resultDir);
#  mesa::create_directory($logLevel, $resultDir);
#
...
##  goodbye if ($x =~ /^q/);
#
#  $mesa_spsSOPUID = rpt_mgr::getSOPInstanceUID($rwlDir . "/mesa/msg1_result.dcm");
#  chomp $mesa_spsSOPUID;
...
sub processTransaction41 {
  ...
  if (! -e $resultDir) {
    print "Trans 41 expected $resultDir created by 38a \n" .
          "This is a programming error; please log a bug report.\n";
    return 1;
  }
  $x = mesa::updateTransactionUID(
  ...
  if( $rtnVal == 1) {
    print "\nMESA Report Creator Failed to complete workitem.\n";
    return 1;
  }
  return 0;
}
...
sub processTransaction39 {
  ...
  print "\nIHE Transaction 39: \n" .
  "Your Report Creator ($test_rptcrt_AE) should send a GPPPS NCREATE to\n" .
  ...
  chomp $mesa_spsSOPUID;
  if ($mesa_spsSOPUID eq "") {
    print "MESA unable to get SOP Instance UID from RWL result in $rwlDir/mesa\n";
    print "This is a bug; please log a bug report.\n";
    return 1;
  }
  print "mesa_spsSOPUID: " . $mesa_spsSOPUID . "\n";
  ...
  print "MESA Report Creator will send N-Create as part of the test process.\n";

  mesa::delete_directory($logLevel, $resultDir);
  mesa::create_directory($logLevel, $resultDir);

  # Create a MESA N-Create message for self test and for comparison
  # with what is created by the system under test.
...
sub processTransaction42a {
  ...
  print "\nIHE Transaction 42: \n" .
  "MESA Report Manager ($mesaRptmgrAE) will send a Performed Work Status Update\n" .
  "(GPPPS NCREATE) to the MESA system at ($mesaIM_AE, localhost, $mesaIMPortDICOM)\n" .
...
sub processTransaction40 {
  ...
  print "\nIHE Transaction 40: \n" .
  "Your Report Creator ($test_rptcrt_AE) should send a GPPPS NSET to\n" .
...
sub processTransaction42b {
  ...
  print "\nIHE Transaction 42: \n" .
  "MESA Report Manager ($mesaRptmgrAE) will send a Performed Work Status Update\n" .
  "(GPPPS NSET) to the MESA system at ($mesaIM_AE, localhost, $mesaIMPortDICOM)\n" .
  "to indicate the performed procedure step completed.\n" .
  "Press <enter> when ready to continue or <q> to quit: ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
  ...
  my ($status, $mesa_ppsSOPUID) = mesa::getGPPPSUID($logLevel, "$resultDir");
  return 1 if ($status != 0);
  chomp $mesa_ppsSOPUID;
  print "mesa_ppsSOPUID: " . $mesa_ppsSOPUID . "\n";

  $rtnVal = send_ppsSet($mesa_rptmgrRWL_AE, $mesaIM_AE, "localhost",
       $mesaIMPortDICOM, $mesa_ppsSOPUID, $setMsg, $resultDir . "/rsp.txt");
  if( $rtnVal == 1) {
    print "\nMESA RPT MGR Failed to send Performed Work Status update (Nset).\n";
    return 1;
  }
...
sub processTransaction46 {
  ...
  print "\nIHE Transaction 46: \n";
  print "MESA will query (itself) for Reporting worklist.\n" .
	"This is the time for your Report Creator to query for Reporting Worklist.\n" .
	"MESA Worklist parameters are ($mesa_rptmgrRWL_AE, $host, $mesa_rptmgrRWL_Port).\n" .
	"Press <enter> when ready to continue or <q> to quit: ";
  my $x = <STDIN>;
  goodbye if ($x =~ /^q/);
...
sub schedule_gpwl {
  ...
  print "MESA will now schedule a workitem as follows: \n";
...
%varnames = mesa::get_config_params("rptcrt_test.cfg");
if (rpt_crt::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in rptcrt_test.cfg\n";
  exit 1;
}

$test_rptcrt_AE = $varnames{"TEST_RPTCRT_AE"};
...
$mesaRptmgrHostHL7    = $varnames{"MESA_RPT_MGR_HOST_HL7"};
$mesaRptmgrPortHL7    = $varnames{"MESA_RPT_MGR_PORT_HL7"};
