...
sub find_candidate_files {
  my $tag = shift(@_);
  my $val = shift(@_);
  print LOG "Report Creator 605-j\n";
...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -p ../../msgs/sr/605-j/sr_605mr.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 605-j/sr_605_mr.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/605-j/sr_605mr.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">605-j/grade_605_mr.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 605-j/grade_605_mr.txt \n";
