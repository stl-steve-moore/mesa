...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -p ../../msgs/sr/603/sr_603cr.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 603/sr_603_cr.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/603/sr_603cr.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">603/grade_603_cr.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 603/grade_603_cr.txt \n";
