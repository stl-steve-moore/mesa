...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -p ../../msgs/sr/606/sr_606ct.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 606/sr_606_ct.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/606/sr_606ct.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">606/grade_606_ct.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 606/grade_606_ct.txt \n";
