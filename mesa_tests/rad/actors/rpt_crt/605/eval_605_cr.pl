...
sub evaluate_candidate_files {
  ...
  foreach $f (@srFiles) {
    ...
    $x .= " -v " if $verbose;
    $x .= " -p ../../msgs/sr/605/sr_605cr.dcm -t 2000:DCMR $f";
    ...
    $x = "$MESA_TARGET/bin/compare_dcm -m 605/sr_605_cr.ini";
    $x .= " -o " if $verbose;
    $x .= " ../../msgs/sr/605/sr_605cr.dcm $f";
    print LOG "$x \n";
    print LOG `$x`;
    if ($? == 0) {
      $rtnValueCompare = 0;
...
open LOG, ">605/grade_605_cr.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 605/grade_605_cr.txt \n";
