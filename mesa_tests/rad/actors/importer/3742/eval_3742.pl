...
sub x_3742_1 {
  print LOG "CTX: Importer 3742.1 \n";
  print LOG "CTX:  Evaluating MPPS for Import\n";

  my @mpps = mesa_get::find_mpps_dir_by_patient_id($logLevel, $titleMPPSMgr, $patientID);
  
  if (scalar(@mpps) == 0) {
    print LOG "ERR: 0 MPPS directory found under MESA Image Manager storage area for $patientID; Are you using the correct patient id 3742?\n";
    return 1;
  }elsif(scalar(@mpps) > 1){
  ...
  my $refMPPS = "$MESA_STORAGE/tmp/3742_A-mpps/mpps.set";
  my $testMPPS = "$mpps[0]/mpps.dcm";
  my @constant = (
     "EQUAL", "",          0 , "0040 0252", "$testMPPS",  "DISCONTINUED", "PPS Status",
     "EQUAL", "",          0 , "0040 0254", "$testMPPS",  "Importation", "PPS Description",
     "EQUAL", "0040 0281", 0,  "0008 0100", "$testMPPS",  "110514", "Code Value",
     "EQUAL", "0040 0281", 0,  "0008 0102", "$testMPPS",  "DCM", "Code Scheme Designator",
     "EQUAL", "0040 0281", 0,  "0008 0104", "$testMPPS",  "Incorrect worklist entry selected", "Code Meaning",
  );
  my @existence = (
  	"0040 0281" , 0, "", "SEQEXIST", "Discontinuation Sequence",
  );
  
  my $errorCount = mesa_evaluate::processExistenceList($logLevel, "", $testMPPS, @existence);
  $errorCount += mesa_evaluate::processConstantList($logLevel, @constant);
  
  #$errorCount += mesa_evaluate::eval_mpps_nSet_importer_status(
	#	$logLevel,
		#$titleMPPSMgr,
		#"$MESA_STORAGE/tmp/3742-mpps/mpps.set",
		#"$mpps[0]/mpps.dcm");
...
sub x_3742_2 {
   ...
   my $refObject = "$MESA_STORAGE/tmp/3742-instances/1001.dcm";
   my $testObject = $x[0];
...
$logLevel = $ARGV[0];
$titleMPPSMgr = $ARGV[1];
open LOG, ">3742/grade_3742.txt" or die "?!";
$diff = 0;

$masterFile = "$MESA_STORAGE/tmp/3742_A-mpps/mpps.crt";
...
print "Logs stored in 3742/grade_3742.txt \n";
