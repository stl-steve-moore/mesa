...
sub checkOBXWithPDF{
  ...
  my $obx5_4 = mesa::getHL7FieldWithSegmentIndex($logLevel, $testORU, "OBX", $index, "5", "4", "Observation Value Data Subtype");

  print main::LOG "CTX: OBX 5: OBX Source Application ID for Observation Value should include a valid ISO OID\n";
  print main::LOG "CTX: OBX 5: OBX Source Application ID for Observation Value in this message is \"$obx5_2\"\n";

  if ($obx5_3 ne "Application"){
    print main::LOG "ERR: OBX 5: OBX Type of Data for Observation Value should be \"Application\"\n";
    print main::LOG "ERR: OBX 5: OBX Type of Date retrieved from HL7 message is \"$obx5_3\"\n";
    print main::LOG "REF: OBX 5: See TF Vol II, Table 4.7-8\n" if ($logLevel >= 4);
    $errorCount++;
  } else {
    print main::LOG "CTX: OBX 5: OBX Type of Data for Observation Value retrieved from HL7 message is \"$obx5_3\"\n";
  }
	
  if ($obx5_4 ne "PDF"){
    print main::LOG "ERR: OBX 5: OBX Data Subtype for Observation Value should be \"PDF\"\n";
    print main::LOG "ERR: OBX 5: OBX Type Subtype for Observation Value retrieved from HL7 message is \"$obx5_4\"\n";
    print main::LOG "REF: OBX 5: See TF Vol II, Table 4.7-8\n" if ($logLevel >= 4);
    $errorCount++;
  } else {
    print main::LOG "CTX: OBX 5: OBX Type Subtype for Observation Value retrieved from HL7 message is \"$obx5_4\"\n";
  }

  print main::LOG "\nCTX: OBX 1: OBX Pointer for Observation Value retrieved from HL7 message is \"$obx5_1\"\n";
  print main::LOG "To retrieve the PDF report, open the URL in any browser.\n";
  print main::LOG "Take a screen capture of the report opened from the browser and paste it to  a word file using the document naming convention of: CompanyName_Product_20520_RM_2005.doc\n";
  print main::LOG "Send the word document to the Project Manager\n\n";
...
#  print main::LOG "CTX: OBX-5 Observation Value should include Study Instance UID as ISO OID in the OBX segment with DICOM Study Instance UID.\n";
#  print main::LOG "CTX: OBX-5 Observation Value should include an ISO OID for the report limited to 64 characters in the OBX segment with Encapsulated PDF.\n";
#  print main::LOG "CTX: Please check OBX-5 which is stored in 20520/grade_20520.txt \n";
...
$logLevel     = $ARGV[0];
open LOG, ">20520/grade_20520.txt" or die "?!";
$diff = 0;
...
print "Logs stored in 20520/grade_20520.txt \n";
exit $diff;
