...
sub processTransaction {
  ...
  if ($trans eq "1") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processTransactionCard1($logLevel, $selfTest, @tokens);
  } else {
...
sub processMESAInternal {
  ...
  if ($trans eq "CARD-SCHEDULE") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processCardiologySchedule($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "UPDATE-PATIENT-DEMO") {
    print "UPDATE-PATIENT-DEMO is not necessary to test the Image Manager \n";       
  } elsif ($trans eq "GEN-SOP-INSTANCES") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::generateSOPInstances($logLevel, $selfTest, @tokens);
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processCardiologyScheduleMPPSTrigger($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "CATH-SCHEDULE-MPPS-TRIGGER-NO-ORDER") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processCardiologyScheduleMPPSTriggerNoOrder($logLevel, $selfTest, @tokens);
  } else {
    die "Unable to process command <$cmd>\n";
  }
  return $rtnValue;
...
sub processMessage {
  ...
  if ($trans eq "CARD-SCHEDULE") {
    shift (@tokens); shift (@tokens);
    $rtnValue = imgmgr::processCardiologyScheduleMessage($logLevel, $selfTest, @tokens);
  } else {
    die "Unable to process command <$cmd>\n";
  }
  return $rtnValue;
...
sub processOneLine {
  ...
  if ($verb[0] eq "TRANSACTION") {
    ...
    print "TEXT_OF is not necessary to test the Image Manager.\n";
    ...
    if ($verb[1] ne "CATH_WORKFLOW") {
      die "This Image Manager script is for the Cardiology Cath profile, not $verb[1]";
    }
