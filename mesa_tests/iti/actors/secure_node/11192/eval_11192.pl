...
sub x_11192_1 {
  print LOG "\nCTX: PMI 11192.1\n";
  print LOG "CTX: Evaluation using ATNA Log Message-Actor Patient Care Resource Assignment Schematron\n";

  my ($logLevel, $file) = @_;
  print LOG "CTX: File $file\n";

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11192/11192_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11192/11192.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11192/11192_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11192/11192.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11192/11192.xsl -OUT 11192/schematron_11192.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11192/schematron_11192.log" or die "Could not open output file 11192/schematron_11192.log";
...
# Main starts here
 ...
 die "Usage: perl 11192/eval_11192.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11192/grade_11192.txt" or die "Could not open output file 11192/grade_11192.txt";
 ...
 my $diff = 0;
 $diff += x_11192_1($logLevel, $fileName);
 ...
 print "Logs stored in 11192/grade_11192.txt\n";
 exit 0;
