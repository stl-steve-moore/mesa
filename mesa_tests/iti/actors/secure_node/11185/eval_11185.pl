...
#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11185/11185_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11185/11185.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11185/11185_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11185/11185.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11185/11185.xsl -OUT 11185/schematron_11185.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11185/schematron_11185.log" or die "Could not open output file 11185/schematron_11185.log";
...
# Main starts here
 ...
 die "Usage: perl 11185/eval_11185.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11185/grade_11185.txt" or die "Could not open output file 11185/grade_11185.txt";
 ...
 my $diff = 0;
 $diff += x_11185_1($logLevel, $fileName);
 ...
 print "Logs stored in 11185/grade_11185.txt\n";
 exit 0;
