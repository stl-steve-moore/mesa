...
sub x_11199_1 {
  print LOG "\nCTX: PMI 11199.1\n";
  print LOG "CTX: Evaluation using ATNA Log Message-Actor QUERY Schematron\n";

  my ($logLevel, $file) = @_;
  print LOG "CTX: File $file\n";

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11199/11199_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11199/11199.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11199/11199_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11199/11199.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11199/11199.xsl -OUT 11199/schematron_11199.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11199/schematron_11199.log" or die "Could not open output file 11199/schematron_11199.log";
...
# Main starts here
 ...
 die "Usage: perl 11199/eval_11199.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11199/grade_11199.txt" or die "Could not open output file 11199/grade_11199.txt";
 ...
 my $diff = 0;
 $diff += x_11199_1($logLevel, $fileName);
 ...
 print "Logs stored in 11199/grade_11199.txt\n";
 exit 0;
