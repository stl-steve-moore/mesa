...
#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11186/11186_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11186/11186.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11186/11186_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11186/11186.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11186/11186.xsl -OUT 11186/schematron_11186.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11186/schematron_11186.log" or die "Could not open output file 11186/schematron_11186.log";
...
# Main starts here
 ...
 die "Usage: perl 11186/eval_11186.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11186/grade_11186.txt" or die "Could not open output file 11186/grade_11186.txt";
 ...
 my $diff = 0;
 $diff += x_11186_1($logLevel, $fileName);
 ...
 print "Logs stored in 11186/grade_11186.txt\n";
 exit 0;
