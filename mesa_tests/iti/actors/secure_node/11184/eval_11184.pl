...
#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11184/11184_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11184/11184.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11184/11184_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11184/11184.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11184/11184.xsl -OUT 11184/schematron_11184.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11184/schematron_11184.log" or die "Could not open output file 11184/schematron_11184.log";
...
# Main starts here
 ...
 die "Usage: perl 11184/eval_11184.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11184/grade_11184.txt" or die "Could not open output file 11184/grade_11184.txt";
 ...
 my $diff = 0;
 $diff += x_11184_1($logLevel, $fileName);
 ...
 print "Logs stored in 11184/grade_11184.txt\n";
 exit 0;
