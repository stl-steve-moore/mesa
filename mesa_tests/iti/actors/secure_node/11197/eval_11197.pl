...
sub x_11197_1 {
  print LOG "\nCTX: PMI 11197.1\n";
  print LOG "CTX: Evaluation using ATNA Log Message-Actor PHI Import Schematron\n";

  my ($logLevel, $file) = @_;
  print LOG "CTX: File $file\n";

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11197/11197_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11197/11197.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11197/11197_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11197/11197.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11197/11197.xsl -OUT 11197/schematron_11197.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11197/schematron_11197.log" or die "Could not open output file 11197/schematron_11197.log";
...
# Main starts here
 ...
 die "Usage: perl 11197/eval_11197.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11197/grade_11197.txt" or die "Could not open output file 11197/grade_11197.txt";
 ...
 my $diff = 0;
 $diff += x_11197_1($logLevel, $fileName);
 ...
 print "Logs stored in 11197/grade_11197.txt\n";
 exit 0;
