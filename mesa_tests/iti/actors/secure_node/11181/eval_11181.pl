...
#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11181/11181_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11181/11181.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11181/11181_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11181/11181.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11181/11181.xsl -OUT 11181/schematron_11181.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11181/schematron_11181.log" or die "Could not open output file 11181/schematron_11181.log";
...
# Main starts here
 ...
 die "Usage: perl 11181/eval_11181.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11181/grade_11181.txt" or die "Could not open output file 11181/grade_11181.txt";
 ...
 my $diff = 0;
 $diff += x_11181_1($logLevel, $fileName);
 ...
 print "Logs stored in 11181/grade_11181.txt\n";
 exit 0;
