...
#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11183/11183_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11183/11183.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11183/11183_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11183/11183.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11183/11183.xsl -OUT 11183/schematron_11183.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11183/schematron_11183.log" or die "Could not open output file 11183/schematron_11183.log";
...
# Main starts here
 ...
 die "Usage: perl 11183/eval_11183.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11183/grade_11183.txt" or die "Could not open output file 11183/grade_11183.txt";
 ...
 my $diff = 0;
 $diff += x_11183_1($logLevel, $fileName);
 ...
 print "Logs stored in 11183/grade_11183.txt\n";
 exit 0;
