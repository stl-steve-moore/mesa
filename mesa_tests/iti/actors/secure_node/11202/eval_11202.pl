...
sub x_11202_1 {
  print LOG "\nCTX: PMI 11202.1\n";
  print LOG "CTX: Evaluation using ATNA Log Message-Actor Study Used Schematron\n";

  my ($logLevel, $file) = @_;
  print LOG "CTX: File $file\n";

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11202/11202_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11202/11202.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 11202/11202_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 11202/11202.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 11202/11202.xsl -OUT 11202/schematron_11202.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "11202/schematron_11202.log" or die "Could not open output file 11202/schematron_11202.log";
...
# Main starts here
 ...
 die "Usage: perl 11202/eval_11202.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">11202/grade_11202.txt" or die "Could not open output file 11202/grade_11202.txt";
 ...
 my $diff = 0;
 $diff += x_11202_1($logLevel, $fileName);
 ...
 print "Logs stored in 11202/grade_11202.txt\n";
 exit 0;
