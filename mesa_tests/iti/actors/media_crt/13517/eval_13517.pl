#!/usr/bin/perl
# Runs the Media Creator test 13517
...
sub x_13517_x {
  ...
  my $error_count = 0;
  print LOG "CTX: XDM Media Creator, Two Submission Sets, Multi Part Doc\n" if ($logLevel >= 3);
  print LOG "CTX: Test 13517 examines 2 Submission Sets each containing a multi part document, created by the Media Creator created by the Media Creator\n" if ($logLevel >= 3);
...
$logLevel = $ARGV[0];
$mountPoint = $ARGV[1];
open LOG, ">13517/grade_13517.txt" or die "?!";
$diff = 0;

$diff = x_13517_x ($logLevel, $mountPoint);
if ($diff != 0) {
 print LOG "Cannot run any other tests if you fail test 1\n";
 print "Cannot run any other tests if you fail test 1\n";
 print "Logs stored in 13517/grade_13517.txt \n";
 exit 1;
}

$diff = x_13517_1 ($logLevel, $mountPoint);
