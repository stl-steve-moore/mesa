#!/usr/bin/perl
# Runs the Media Creator test 13516
...
sub x_13516_x {
  ...
  my $error_count = 0;
  print LOG "CTX: XDM Media Creator, Two Submission Sets, Single Part Doc\n" if ($logLevel >= 3);
  print LOG "CTX: Test 13516 examines 2 Submission Sets each containing a single part document created by the Media Creator\n" if ($logLevel >= 3);
...
$logLevel = $ARGV[0];
$mountPoint = $ARGV[1];
open LOG, ">13516/grade_13516.txt" or die "?!";
$diff = 0;

$diff = x_13516_x ($logLevel, $mountPoint);
if ($diff != 0) {
 print LOG "Cannot run any other tests if you fail test 1\n";
 print "Cannot run any other tests if you fail test 1\n";
 print "Logs stored in 13516/grade_13516.txt \n";
 exit 1;
}

$diff = x_13516_1 ($logLevel, $mountPoint);
