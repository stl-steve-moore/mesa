#!/usr/bin/perl
# Runs the Media Creator test 13514
...
sub x_13514_x {
  ...
  my $error_count = 0;
  print LOG "CTX: XDM Media Creator, One Submission Set, Single Part Doc\n" if ($logLevel >= 3);
  print LOG "CTX: Test 13514 examines a Submission Set containing a single document, created by the Media Creator\n" if ($logLevel >= 3);
...
$logLevel = $ARGV[0];
$mountPoint = $ARGV[1];
open LOG, ">13514/grade_13514.txt" or die "?!";
$diff = 0;

$diff = x_13514_x ($logLevel, $mountPoint);
if ($diff != 0) {
 print LOG "Cannot run any other tests if you fail test 1\n";
 print "Cannot run any other tests if you fail test 1\n";
 print "Logs stored in 13514/grade_13514.txt \n";
 exit 1;
}

$diff = x_13514_1 ($logLevel, $mountPoint);
