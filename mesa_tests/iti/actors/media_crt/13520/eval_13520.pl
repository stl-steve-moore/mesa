...
sub x_13520_1 {
  print LOG "\nCTX: Media Creator 13520.1\n";
  print LOG "CTX: Evaluation using ATNA-Export Schematron\n";

  my ($logLevel, $file) = @_;
  print LOG "CTX: File $file\n";

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 13520/13520_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 13520/13520.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 13520/13520_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 13520/13520.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 13520/13520.xsl -OUT 13520/schematron_13520.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "13520/schematron_13520.log" or die "Could not open output file 13520/schematron_13520.log";
...
# Main starts here
 ...
 die "Usage: perl 13520/eval_13520.pl <log level> " if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];

 open LOG, ">13520/grade_13520.txt" or die "Could not open output file 13520/grade_13520.txt";
 ...
 my $diff = 0;
 $diff += x_13520_1($logLevel, $fileName);
 ...
 print "Logs stored in 13520/grade_13520.txt\n";
 exit 0;
