...
sub processTransaction1 {
  print "Transaction 1 not necessary to test the PD Consumer\n";
  return 0;
}

sub processTransaction2 {
  print "Transaction 2 not necessary to test the PD Consumer\n";
  return 0;
}
...
sub processTransaction {
  ...
  if ($trans eq "ITI-1") {
    ...
    $rtnValue = pd_consumer::processTransaction8($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "ITI-9") {
    $rtnValue = 1;
  } elsif ($trans eq "ITI-21") {
    $rtnValue = 1;
    shift (@tokens); shift (@tokens);
    $rtnValue = pd_consumer::processTransaction21($logLevel, $selfTest, @tokens);
  } else {
...
my $lineNumber = 1;
print `perl scripts/clear_server_storage.pl`;
die "Could not reset servers \n" if ($?);
