...
sub doQuery{
  $ldif = Net::LDAP::LDIF->new( "./11520/11520.log", "w", onerror => 'undef' );
  $recordNo = 1;
  $msg = $ldap->search(filter=>"(cn=Alexander^Matthew*)", base=>$pwpLDIFBase);
  @entries = $msg->entries;
...
open (FILEHANDLE, ">./11520/pwp_11520_log.txt") or die "no such file";
print FILEHANDLE "Test Number: 11520 \nPort Number: $pwpPortTest \nRecorded at: ", $timenow;
