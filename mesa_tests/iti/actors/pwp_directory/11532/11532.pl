...
sub doQuery{
  $ldif = Net::LDAP::LDIF->new( "./11532/11532.log", "w", onerror => 'undef' );
  $recordNo = 1;
  $msg = $ldap->search(filter=>"(employeeNumber=2030)", base=>$pwpLDIFBase);
  @entries = $msg->entries;
...
open (FILEHANDLE, ">./11532/pwp_11532_log.txt") or die "no such file";
print FILEHANDLE "Test Number: 11532 \nPort Number: $pwpPortTest \nRecorded at: ", $timenow;
