...
sub doQuery{
  $ldif = Net::LDAP::LDIF->new( "./11530/11530.log", "w", onerror => 'undef' );
  $recordNo = 1;
  $msg = $ldap->search(filter=>"(Initials=cod)", base=>$pwpLDIFBase);
  @entries = $msg->entries;
...
open (FILEHANDLE, ">./11530/pwp_11530_log.txt") or die "no such file";
print FILEHANDLE "Test Number: 11530 \nPort Number: $pwpPortTest \nRecorded at: ", $timenow;
