...
sub doQuery {
  $ldif = Net::LDAP::LDIF->new( "./11533/11533.log", "w", onerror => 'undef' );
  $recordNo = 1;
  $msg = $ldap->search(filter=>"(mail=fwe\@ihepwp.org)", base=>$pwpLDIFBase);
  @entries = $msg->entries;
...
open (FILEHANDLE, ">./11533/pwp_11533_log.txt") or die "no such file";
print FILEHANDLE "Test Number: 11533 \nPort Number: $pwpPortTest \nRecorded at: ", $timenow;
