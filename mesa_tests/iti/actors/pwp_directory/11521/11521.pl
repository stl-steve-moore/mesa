...
sub doQuery{
  $ldif = Net::LDAP::LDIF->new( "./11521/11521.log", "w", onerror => 'undef' );
  $recordNo = 1;
  $msg = $ldap->search(filter=>"(sn=MO*)", base=>$pwpLDIFBase);
  @entries = $msg->entries;
...
open (FILEHANDLE, ">./11521/pwp_11521_log.txt") or die "no such file";
print FILEHANDLE "Test Number: 11521 \nPort Number: $pwpPortTest \nRecorded at: ", $timenow;
