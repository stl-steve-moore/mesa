...
# Runs the Patient Demographics Source scripts interactively
...
sub processTransaction {
  ...
  if ($trans eq "ITI-1") {
    ...
    shift (@tokens); shift (@tokens);
    $rtnValue = pds_pam::processTransaction8($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "ITI-30") {
    shift (@tokens); shift (@tokens);
    $rtnValue = pds_pam::processTransaction30($logLevel, $selfTest, @tokens);
  } else {
...
if (pds_pam::test_var_names(%varnames) != 0) {
  print "Some problem with the variables in id_source.cfg\n";
  exit 1;
}
...
print "MESA Patient Demographics Consumer Port HL7 = $mesaPdcPamPortHL7\n";
