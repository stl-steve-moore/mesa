...
sub processTransaction {
  ...
  if ($trans eq "ITI-1") {
    ...
    $rtnValue = xref_cons::processTransaction1($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "ITI-2") {
    shift (@tokens); shift (@tokens);
    $rtnValue = processTransaction2(@tokens);
  } elsif ($trans eq "ITI-8") {
    shift (@tokens); shift (@tokens);
    $rtnValue = xref_cons::processTransaction8($logLevel, $selfTest, @tokens);
  } elsif ($trans eq "ITI-9") {
    shift (@tokens); shift (@tokens);
    $rtnValue = xref_cons::processTransaction9($logLevel, $selfTest, @tokens);
  } else {
