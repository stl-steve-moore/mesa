...
sub x_40102_1 {
  print LOG "\nCTX: PCC 40102.1\n";
  print LOG "CTX: Evaluation using XPHR Schematron\n";

  my ($logLevel, $file) = @_;

#  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 40102/40102_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 40102/40102.xsl -PARAM diagnose yes";
  my $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN 40102/40102_schematron.xml -XSL schematron/schematron-basic.xsl -OUT 40102/40102.xsl -PARAM diagnose yes";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;

  $x = "java -cp $MESA_TARGET/lib/xalan.jar org.apache.xalan.xslt.Process -IN $file -XSL 40102/40102.xsl -OUT 40102/schematron_40102.log";
  print LOG "CTX: $x\n" if ($logLevel >= 3);
  print LOG `$x`;
...
sub appendResult {
  open RESULT, "40102/schematron_40102.log" or die "Could not open output file 40102/schematron_40102.log";
...
# Main starts here
 ...
 die "Usage: perl 40102/eval_40102.pl <log level> FILE" if (scalar(@ARGV) < 1);
 $logLevel = $ARGV[0];
 check_environment();

 open LOG, ">40102/grade_40102.txt" or die "Could not open output file 40102/grade_40102.txt";
 ...
 my $diff = 0;
 $diff += x_40102_0($logLevel, $fileName);
 if ($diff == 0) {
  $diff += x_40102_1($logLevel, $fileName);
 }
 ...
 print "Logs stored in 40102/grade_40102.txt\n";
 exit 0;
